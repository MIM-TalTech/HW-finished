    ...
    [TestClass] public class SortedRepositoryTests : AbstractClassTests<SortedRepository<Measure, MeasureData>,
        ...
        [TestMethod] public void SetSortingTest() {
            void test(IQueryable<MeasureData> d, string sortOrder)
            {
                ...
                Assert.IsTrue(str.Contains($"HW4.Data.Quantity.MeasureData]).OrderByDescending(x => Convert(x.{sortOrder}, Object))"));
                obj.SortOrder = sortOrder;
                set = obj.addSorting(d);
                Assert.IsNotNull(set);
                Assert.AreNotEqual(d, set);
                str = set.Expression.ToString();
                Assert.IsTrue(str.Contains($"HW4.Data.Quantity.MeasureData]).OrderBy(x => Convert(x.{sortOrder}, Object))"));
            }
        ...
        [TestMethod] public void SetOrderByTest() {
            void test(IQueryable<MeasureData> d, Expression<Func<MeasureData, object>> e, string expected) {
                ...
                Assert.IsTrue(set.Expression.ToString()
                    .Contains($"HW4.Data.Quantity.MeasureData]).OrderByDescending({expected})"));
